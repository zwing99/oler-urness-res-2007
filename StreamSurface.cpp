...
void StreamSurface::calculateDrawPts(int surfaceNum)
{
	...
	{
		factor1 = 1;
		factor2 = (double)size21/size11;
		limit = size11;
	}
	
	drawpts[surfaceNum][0][0] = pts1[0];
	drawpts[surfaceNum][0][1] = pts2[2];
	drawpts[surfaceNum][0][2] = pts1[2];
	...
	{
		drawpts[surfaceNum][numOfPolygons[surfaceNum]][0] = pts1[(int)(factor1*i)];
		cout<<(int)(factor1*i)<< " "<<(int)(factor2*i)<<endl;
		drawpts[surfaceNum][numOfPolygons[surfaceNum]][1] = pts2[(int)(factor2*i)];
		drawpts[surfaceNum][numOfPolygons[surfaceNum]][2] = pts1[(int)(factor1*(i+2))];
		normals[surfaceNum][numOfPolygons[surfaceNum]] = 
		        calculateNorm(pts1[(int)(factor1*i)],pts2[(int)(factor2*i)],pts1[(int)(factor1*(i+2))]);
		numOfPolygons[surfaceNum]++;
		
		drawpts[surfaceNum][numOfPolygons[surfaceNum]][0] = pts2[(int)(factor2*i)];
		drawpts[surfaceNum][numOfPolygons[surfaceNum]][1] = pts2[(int)(factor2*(i+2))];
		drawpts[surfaceNum][numOfPolygons[surfaceNum]][2] = pts1[(int)(factor1*(i+2))];
		normals[surfaceNum][numOfPolygons[surfaceNum]] = 
		        calculateNorm(pts2[(int)(factor2*i)],pts2[(int)(factor2*(i+2))],pts1[(int)(factor1*(i+2))]);
		numOfPolygons[surfaceNum]++;
	}
	/*
	if(size11 > size21)
