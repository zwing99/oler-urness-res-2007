...
StreamLine* my_stream = new StreamLine(point(10,10,10),currentLat);
	...
	for(i=0; i<10; i+=1)
	{
		for(j=0; j<10; j+=1)
		{
			for(k=0; k<10; k+=1)
			{
				...
				if(i == 0 && j == 0 && k == 0)
					{cout<<myVec.getU()<<","<<myVec.getV()<<","<<myVec.getW()<<endl;
					cout<<(myVec.getU()+1)/2<<","<<(myVec.getV()+1)/2<<","<<(myVec.getW()+1)/2<<endl;}
				if(dipolecoloring)
...
void display(void)
{
	...
	glPushMatrix();
		...
		glPushMatrix();
			glScalef(1.0/sizex,1.0/sizey,1.0/sizez);
			glColor3f(1,0,0);
			StreamLine* myStream = my_stream;
			point* myPts = myStream->getPoints();
			int numOfPoints = myStream->getSize();
			int numOfPoints2 = myStream->getSize2();
			int sizeoflat = currentLat->getSize()[0];
