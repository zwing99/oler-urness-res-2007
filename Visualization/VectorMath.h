...
inline void derivative3D(unsigned char *magV1, 
										float *gradV3, 
		...
		for(int j = 0; j < (sy); ++j){
			for(int k = 0; k < (sx); ++k){
				if((k<2)||(k>sx-3)||(j<2)||(j>sy-3)||(i<2)||(i>sz-3)){
					...
					tmp[i*sx*sy + j*sx + k] = (float)sqrt((float)dx*dx + dy*dy + dz*dz);
...
inline void derivative3DVGH(float *gradV3, 
										   const int sx, const int sy, const int sz, 
										   const unsigned char *dat)
{
		...
		for(int j = 0; j < (sy); ++j){
			for(int k = 0; k < (sx); ++k){
				if((k<2)||(k>sx-3)||(j<2)||(j>sy-3)||(i<2)||(i>sz-3)){
					...
					tmp[i*sx*sy + j*sx + k] = (float)sqrt((float)(dx1+dx2)*(dx1+dx2) + (dy1+dy2)*(dy1+dy2) + (dz1+dz2)*(dz1+dz2));
					maxm = maxm < tmp[i*sx*sy + j*sx + k] ? tmp[i*sx*sy + j*sx + k] : maxm;
				}
			}
		}
...
inline void blurV3D(float *gradV3, float w0, float w1, float w2, float w3,
...
inline void blurVARB(unsigned char *dataV, float w0, float w1, float w2, float w3,
...
inline void blurV3DN(float *gradV3, float w0, float w1, float w2, float w3,
							 int sx, int sy, int sz){
	float *tmp = new float[sx*sy*sz*3];
	int stx = 3;
	int sty = sx*3;
